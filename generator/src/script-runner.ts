...
createAndSavePerformanceDymo(AUDIO_PATH, AUDIO_PATH+'save.json')
  .then(() => console.log('done'))
  .then(() => server.close())
  .catch(e => console.log(e));

function createAndSaveDoublePerformanceDymo(audioPaths: string[], dymoOutPath: string): Promise<any> {
  return Promise.all(audioPaths.map(p => getFilesInDir(p, ['wav'])))
    ...
    .then(jsonld => writeFile(jsonld, dymoOutPath))
    .catch(e => console.log(e));
}

function createAndSavePerformanceDymo(audioPath: string, dymoOutPath: string): Promise<any> {
